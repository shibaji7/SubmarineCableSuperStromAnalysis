...
def read_dataset(
    base_path: str = "data/1958/{x}[ESK]-rescale-HR.csv",
) -> pd.DataFrame:
    """
    ...
    data.Time = data.Time - dt.timedelta(minutes=10)
    data = data[data.Time < data.Time.iloc[-1] - dt.timedelta(minutes=2)]
    data.X = data.X*2.
    # data.Y = data.Y*6

    data = data[
        (data.Time >= dt.datetime(1958, 2, 11, 1))
        & (data.Time < dt.datetime(1958, 2, 11, 5))
    ]  # Filter for the storm period
    # Save processed data
    print(len(data))
    data = data.rename(columns=dict(Time="Date"))
    data[["Date", "X", "Y", "Z", "F"]].to_csv(
        "data/1958/compiledLR.csv", float_format="%g", index=False
    )
    logger.info("Data saved to data/1958/compiledLR.csv")
    ...
    sp.save_fig("figures/1958/1958.data.LR.png")
    sp.close()
    return data
...
def compile_1958(datafile=["data/1958/compiledLR.csv"]):
    """
    ...
    model.plot_TS_with_others(
        ...
        date_lim=[dt.datetime(1958, 2, 11, 1), dt.datetime(1958, 2, 11, 5)],
        fig_title="SCUBAS (Esk) / Time: UT since 1 UT on 11 Feb 1958",
        text_size=10,
    )
    # model.plot_profiles(
